...
const ACTIVITY_THROTTLE_MS = 10000; // 10초마다 한 번씩만 세션 업데이트
...
function generateSecureSessionId(): string {
  ...
  if (typeof crypto !== 'undefined' && crypto.randomUUID) {
    ...
    // TODO: Consider server-side session ID generation for environments without crypto
...
function startSessionHeartbeat() {
  ...
  // HEARTBEAT_INTERVAL_MS마다 세션 갱신 (페이지가 보이는 동안)
  heartbeatInterval = window.setInterval(() => {
    ...
    // 마지막 하트비트로부터 HEARTBEAT_INTERVAL_MS 이상 지났을 때만
...
if (typeof window !== 'undefined') {
  ...
  const handleActivity = () => {
    const now = Date.now();
    // ACTIVITY_THROTTLE_MS마다 한 번씩만 업데이트 (과도한 쓰기 방지)
