...
### 새 기능 (v0.1.2)
- 🔐 **고유 세션 ID 시스템**: 암호학적으로 안전한 세션 식별자 생성
- 📊 **세션 추적**: 세션 시작, 지속 시간, 페이지뷰 자동 추적
- ⏱️ **세션 수명주기 관리**: 30분 비활성 후 자동 새 세션 생성
- 🔄 **세션 지속성**: sessionStorage 기반으로 새로고침 시에도 유지

### 이전 업데이트 (v0.1.1)
- ✨ 초기 보고 모드 추가 (`initialReport`: none/progress/snapshot/guarded)
- 🛡️ 초기 가드 시스템 (`initialGuard`로 안정적인 첫 보고)
- 🎨 사용자 상호작용 감지 기능
- 📦 스크롤 컨테이너 지원 개선
- 🔧 window.StlTracker 글로벌 객체 제공

### 성능 최적화
- 🚀 프로덕션 빌드 시 자동 console 제거
- ⚡ Date.now() 호출 최소화로 성능 향상
- 🧹 메모리 누수 방지 (이벤트 리스너 정리)
- 📉 번들 크기 최적화 (트리쉐이킹 개선)
- 🔄 중복 코드 제거 및 유틸리티 통합
- 💾 localStorage 관리 개선 (자동 크기 제한)
